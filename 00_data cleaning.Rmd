...
table(mons$Monitoring.Objective,
      mons$Measurement.Scale)

st_write(mons, "data/mons_clean.shp", append = FALSE)

table(mons$criteria)
100 * table(mons$criteria, mons$Measurement.Scale) / 1074
...
ST <- c("PR")#"AL", "AK", "AZ", "AR", "CA", "CO", "CT", "DE", "DC", 
...
write.csv(acs_data, "data/acs_data_pr.csv", row.names = FALSE)
...
# population centers
We don't just want block groups; we want to know where the population is in that block group. We'll use population-weighted centroids later on to see if the measurement scale of EPA monitors intersects with these locations — in other words, is the majority of a population in an area likely captured within the monitor's measurement area.
...
count_pm_lower <- data.frame(t(table(measured_pm_lower$geoid)))[, 2:3] |>
  rename("geoid" = Var2) |>
  rename("n_lower_pm" = Freq)
count_pm <- merge(count_pm_upper, count_pm_lower, by = "geoid", all = TRUE)

count_no2_upper <- data.frame(t(table(measured_no2_upper$geoid)))[, 2:3] |>
  rename("geoid" = Var2) |>
  rename("n_upper_no2" = Freq)
count_no2_lower <- data.frame(t(table(measured_no2_lower$geoid)))[, 2:3] |>
  rename("geoid" = Var2) |>
  rename("n_lower_no2" = Freq)
count_no2 <- merge(count_no2_upper, count_no2_lower, by = "geoid", all = TRUE)

count_so2_upper <- data.frame(t(table(measured_so2_upper$geoid)))[, 2:3] |>
  rename("geoid" = Var2) |>
  rename("n_upper_so2" = Freq)
count_so2_lower <- data.frame(t(table(measured_so2_lower$geoid)))[, 2:3] |>
  rename("geoid" = Var2) |>
  rename("n_lower_so2" = Freq)
count_so2 <- merge(count_so2_upper, count_so2_lower, by = "geoid", all = TRUE)

count_co_upper <- data.frame(t(table(measured_co_upper$geoid)))[, 2:3] |>
  rename("geoid" = Var2) |>
  rename("n_upper_co" = Freq)
count_co_lower <- data.frame(t(table(measured_co_lower$geoid)))[, 2:3] |>
  rename("geoid" = Var2) |>
  rename("n_lower_co" = Freq)
count_co <- merge(count_co_upper, count_co_lower, by = "geoid", all = TRUE)

count_o3_upper <- data.frame(t(table(measured_o3_upper$geoid)))[, 2:3] |>
  rename("geoid" = Var2) |>
  rename("n_upper_o3" = Freq)
count_o3_lower <- data.frame(t(table(measured_o3_lower$geoid)))[, 2:3] |>
  rename("geoid" = Var2) |>
  rename("n_lower_o3" = Freq)
count_o3 <- merge(count_o3_upper, count_o3_lower, by = "geoid", all = TRUE)

count_pb_upper <- data.frame(t(table(measured_pb_upper$geoid)))[, 2:3] |>
  rename("geoid" = Var2) |>
  rename("n_upper_pb" = Freq)
count_pb_lower <- data.frame(t(table(measured_pb_lower$geoid)))[, 2:3] |>
  rename("geoid" = Var2) |>
  rename("n_lower_pb" = Freq)
count_pb <- merge(count_pb_upper, count_pb_lower, by = "geoid", all = TRUE)
...
acs_merge <- acs_data |>
  merge(count_pm, by = "geoid", all.x = TRUE) |>
  merge(count_no2, by = "geoid", all.x = TRUE) |>
  merge(count_so2, by = "geoid", all.x = TRUE) |>
  merge(count_co, by = "geoid", all.x = TRUE) |>
  merge(count_o3, by = "geoid", all.x = TRUE) |>
  # merge(count_pb, by = "geoid", all.x = TRUE) |>
  mutate(n_upper_pb = 0)|>#ifelse(is.na(n_upper_pb), 0, n_upper_pb)) |> # 0)|>
  mutate(n_lower_pb = 0)|>#ifelse(is.na(n_lower_pb), 0, n_lower_pb)) |> # 0)|>
  mutate(n_lower_no2 = ifelse(is.na(n_lower_no2), 0, n_lower_no2)) |>
  mutate(n_upper_no2 = ifelse(is.na(n_upper_no2), 0, n_upper_no2)) |>
  mutate(n_lower_so2 = ifelse(is.na(n_lower_so2), 0, n_lower_so2)) |>
  mutate(n_upper_so2 = ifelse(is.na(n_upper_so2), 0, n_upper_so2)) |>
  mutate(n_lower_pm = ifelse(is.na(n_lower_pm), 0, n_lower_pm)) |>
  mutate(n_upper_pm = ifelse(is.na(n_upper_pm), 0, n_upper_pm)) |>
  mutate(n_lower_co = ifelse(is.na(n_lower_co), 0, n_lower_co)) |>
  mutate(n_upper_co = ifelse(is.na(n_upper_co), 0, n_upper_co)) |>
  mutate(n_lower_o3 = ifelse(is.na(n_lower_o3), 0, n_lower_o3)) |>
  mutate(n_upper_o3 = ifelse(is.na(n_upper_o3), 0, n_upper_o3))
...
write.csv(acs_wo_geom, "data/7020_data_count_PR.csv", row.names = FALSE)
# write.csv(acs_wo_geom, "data/7020_data_count.csv", row.names = FALSE)
st_write(acs_geom, "data/7020_geom_PR.shp", append = TRUE)
